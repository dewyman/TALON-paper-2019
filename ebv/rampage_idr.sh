...
echo "-- Removing any spike-ins from bed files..."
set -x
grep "^chr" $peaks_a_file > peaks_a_clean.bed
grep "^chr" $peaks_b_file > peaks_b_clean.bed
set -x

echo "-- Running IDR..."
set -x
idr --input-file-type bed --rank 7 --plot --verbose --samples peaks_a_clean.bed peaks_b_clean.bed 2>&1 | tee idr_summary.txt
