...
def getOptions():
    ...
    parser.add_option("--pas", dest = "pas",
        help = "PolyA-seq reads mapped to the genome in BAM format", metavar = "FILE", type = "string")
    ...
    parser.add_option("--n", dest = "n_reads",
        help = "Number of reads that consitute PAS support",
        type = "int")
...
def main():
    ...
    try:
        out = outprefix
        subprocess.check_output(["python", script_dir +"/parse_bedtools_cov_output.py",
        ...
        print(e)
        sys.exit("Something went wrong during the parse_bedtools_cov_output.py run")
