main <-function() {
    ...
    # print(unique(abundance_table$gene_novelty))
    ...
    merged_abundances$novelty <- factor(merged_abundances$novelty, levels = c("Known", "Novel"))

    # print(head(merged_abundances))
    # quit()

    # Plot expression scatterplots
    expression_by_status(merged_abundances, d1, d2, opt, opt$outdir, color_vec, opt$celltype, opt$lsr, opt$corr_labs, opt$regression_line, opt$d1_label, opt$d2_label)
}

expression_by_status <- function(merged_abundances, d1, d2, options, outdir, color_vec, celltype, lsr, corr_labs, regression_line, d1_label, d2_label) {
    ...
    # print(head(merged_abundances$data1.TPM))
    # print(head(merged_abundances$data2.TPM))

    # print(which(is.na(merged_abundances$data1.TPM)))
    # print(which(is.na(merged_abundances$data2.TPM)))
    ...
    xlabel <- paste("log2(TPM+1) in ", celltype, " ", d1_label, sep="")
    ylabel <- paste("log2(TPM+1) in ", celltype, " ", d2_label, sep="")
...
parse_options <- function() {
    ...
    option_list <- list(
        ...
        make_option(c("--d1_label"), action = "store", dest = "d1_label",
                    help = "Label for data type on dataset d1. ie PacBio, Rep1 etc"), 
        make_option(c("--d2_label"), action = "store", dest = "d2_label",
                    help = "Label for data type on dataset d2. ie PacBio, Rep1 etc"), 
